  ...
  nodesEl.innerHTML = deptList.map((d) => {
    ...
    return `<div class="track-node ${cls}" style="--node-color:${d.color};">
      ...
      <div class="meta">
        ...
        <div class="pct mono"><span>${completePct}% complete</span><span>${inProgressPct}% in progress</span></div>
...
function renderKPIs(totals) {
  ...
  const cards = [
    ...
    { label: "Completed",           value: totals.complete,          accent: "var(--green)",   icon: ic('<circle cx="12" cy="12" r="9"/><path d="M8 12.5l2.8 2.8L16 10"/>') },
    { label: "In Progress",         value: totals.inProgress,        accent: "var(--amber)",   icon: ic('<circle cx="12" cy="12" r="9"/><path d="M12 7v5l3.2 2"/>') },
    { label: "Not Started",         value: totals.notStarted,        accent: "var(--ink-3)",   icon: ic('<circle cx="12" cy="12" r="9" stroke-dasharray="3 3"/>') },
    { label: "Start Delayed",       value: totals.startDelayed,      accent: "var(--magenta)", icon: ic('<path d="M12 4.5L21 19H3z"/><path d="M12 10v4"/><path d="M12 16.6v.1"/>') },
...
function renderDonuts(totals) {
  const statusSegs = [
    ...
    { label: "Not Started", value: totals.notStarted, color: "var(--st-pending)" },
  ];
  drawDonut("donut-status", statusSegs.map(s=>({...s,color:s.color.replace('var(--green)','#00874A').replace('var(--orange)','#C98A00').replace('var(--ink-faint)','#D8D2C6')})), totals.items);
...
function pctColor(p) {
  if (p === null) return { fg: "var(--ink-3)", bg: "transparent" };
  if (p >= 75)    return { fg: "var(--green)",   bg: "var(--green-soft)" };
  if (p >= 35)    return { fg: "var(--cyan)",    bg: "var(--cyan-soft)" };
  if (p > 0)      return { fg: "var(--amber)",   bg: "var(--amber-soft)" };
  return { fg: "var(--ink-3)", bg: "var(--surface-sunk)" };
}
